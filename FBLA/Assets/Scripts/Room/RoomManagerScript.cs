...
/*
 * RoomManagerScript is the script that manages room
 * interactions, from UI inputs to player and enemy movements and
 * data changes. Thus, all components of a room interact with RoomManagerScript.
 */
public class RoomManagerScript : MonoBehaviour
{
	...
	public IEnumerator processRight() {
		...
		DataModel.changeScore(Mathf.RoundToInt(time) * 100 * (DataModel.getDifficulty()+1));
		//player attacks
		...
		if (DataModel.getEnemyHealth () <= 0) {
			...
			//gets new question
			DataModel.setQuestion(QuestionManager.getRandomQuestion (DataModel.getDifficulty () + DataModel.getEnemyType()));
		}
	}
