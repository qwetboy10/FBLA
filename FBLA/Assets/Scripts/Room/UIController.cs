...
/*
 * UIController handles the many interactions of UI,
 * from button presses to fade in and fade out overlays. ALl
 * UI components interact with UIController, which in turn may
 * call various methods in RoomManagerScript when a question is answered correctly
 * or a potion is used, for example.
 */
public class UIController : MonoBehaviour
{
	...
	//back menu button
    public Button back;
	//text for treasure chest rewards
	...
	//utility methods for each button that calls pressButton for its index
    ...
    public void pressButton(int b)
    {
        if(menu)
        {
            ...
            if(b==0)
            {
                menu = false;
                updateUI();
                back.GetComponent<ButtonHover>().OnPointerExit(null);
            }
            //Potion
            if(b==1)
            {
                if(DataModel.getPotions()>=1)
                {
                    ...
                    buttons[1].GetComponent<ButtonHover>().OnPointerExit(null);
            ...
            if(b==2)
            {
                if(DataModel.getCoins()>=10 && !DataModel.getQuestion().hinted)
                {
                    ...
                    menu = false;
                    updateUI();
                    buttons[2].GetComponent<ButtonHover>().OnPointerExit(null);
    ...
    public IEnumerator showDamage(int i)
    {
        ...
        while (cur < GameConstants.damageNumberTime)
        {
            ...
            damageText.color = Color.Lerp(damageTextColor, final, cur / GameConstants.damageNumberTime);
